...
def save_progress(done_ids: set):
    PROG.write_text(json.dumps({"done_ids": sorted(done_ids)}, indent=2))

def load_progress()->set:
    if PROG.exists():
        return set(json.loads(PROG.read_text()).get("done_ids", []))
    return set()
...
def annotate(ex):
    ...
    while True:
        ...
        if choice == "u":
            return {
              "id": ex["id"], "query": ex["query"], "conflict_type": ctype, "docs": ex["docs"],
              "notes": notes, "final": {"style_hint": style, "answer": "Unknown based on provided documents.", "evidence": [], "abstain": True},
              "trace_type":"summarized","think":"<think>Per-doc notes then abstain because no support.</think>"
            }
        if choice == "a":
            ...
            return {
              "id": ex["id"], "query": ex["query"], "conflict_type": ctype, "docs": ex["docs"],
              "notes": notes, "final": {"style_hint": style, "answer": ans, "evidence": cites, "abstain": False},
              "trace_type":"summarized","think":"<think>Summarized per-doc notes precede the final answer.</think>"
            }

def main():
    if not INP.exists():
        raise SystemExit("Run sample_50.py first.")
    done = load_progress()
    OUT.parent.mkdir(parents=True, exist_ok=True)
    items = [ex for ex in read_jsonl(INP) if ex["id"] not in done]
    print(f"{len(done)} already annotated, {len(items)} to go.")
    for ex in items:
        print("\n"+"="*100)
        ...
        done.add(ex["id"]); save_progress(done)
        print(f"Saved {ex['id']} ✔")
    print("All done ✅")
